...
const NAV = [
  ...
  { to: '/doctor/prescriptions/new', label: 'New Rx', icon: FilePlus },
  { to: '/doctor/prescriptions', label: 'History', icon: ClipboardList },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV.map(({ to, label, icon: Icon }) => (
            <Link key={to} to={to}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${pathname === to
